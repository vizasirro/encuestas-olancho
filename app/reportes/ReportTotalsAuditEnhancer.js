...
function readFilters(){
 ...
 const labels=root?[...root.querySelectorAll('label')]:[];const select=t=>labels.find(l=>l.textContent?.trim().startsWith(t))?.querySelector('select');const dates=root?[...root.querySelectorAll('input[type="date"]')]:[];
...
function rowMeta(x){return{estado:first(x,['estado','status']),tipo:first(x,['tipo_encuesta','encuesta_codigo','tipo','codigo_encuesta']),ecor:first(x,['ecor','ecor_nombre']),municipio:first(x,['municipio','municipio_nombre']),establecimiento:first(x,['establecimiento_nombre','establecimiento','nombre_establecimiento']),fecha:first(x,['enviada_at','fecha_envio','finalizada_at','iniciada_at']),id:first(x,['sesion_id','id','session_id'])}}
...
export default function ReportTotalsAuditEnhancer(){
 ...
 useEffect(()=>{
  ...
  const detailRows=detalle.filter(x=>applies({...x,estado:'ENVIADA'},filters));box.dataset.totalRespuestas=String(detailRows.length);
  const detailMap=new Map();detailRows.forEach(x=>{const id=String(x.sesion_id||'');if(id)detailMap.set(id,x)});const conDetalle=detailMap.size;
  const listRows=lista.filter(x=>applies(x,filters));const listMap=new Map();listRows.forEach((x,i)=>{const m=rowMeta(x);listMap.set(String(m.id||`row-${i}`),x)});const fuenteSesiones=listMap.size;
  const fuenteValida=fuenteSesiones>0||conDetalle===0;const total=fuenteValida?fuenteSesiones:conDetalle;const base=fuenteValida?[...listMap.values()]:[...detailMap.values()];const amb=base.filter(x=>isAmb(rowMeta(x).tipo||x.tipo_encuesta)).length;const hosp=base.filter(x=>isHosp(rowMeta(x).tipo||x.tipo_encuesta)).length;
  box.dataset.ambulatorias=String(amb);box.dataset.hospitalizacion=String(hosp);
  ...
  if(!fuenteValida&&conDetalle>0){box.innerHTML=`<div style="font-weight:900;font-size:16px;margin-bottom:8px">Control de totales</div><div style="font-size:28px;font-weight:900">${conDetalle}</div><small>Encuestas con detalle de respuestas dentro de los filtros actuales</small><div style="margin-top:10px;padding:9px 10px;border-radius:8px;background:#fff4df;color:#7a4800;font-weight:700">⚠️ La fuente de sesiones enviadas no coincidió correctamente con el filtro. Se mantiene el total analítico comprobable (${conDetalle}).</div>`;return}
  const sinDetalle=Math.max(0,total-conDetalle);
  box.innerHTML=`<div style="font-weight:900;font-size:16px;margin-bottom:8px">Totales conciliados · fuente de sesiones enviadas</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px"><div><div style="font-size:28px;font-weight:900">${total}</div><small>TOTAL VÁLIDAS</small></div><div><div style="font-size:28px;font-weight:900">${amb}</div><small>Ambulatorias</small></div><div><div style="font-size:28px;font-weight:900">${hosp}</div><small>Hospitalización</small></div><div><div style="font-size:28px;font-weight:900">${conDetalle}</div><small>Con detalle de respuestas</small></div><div><div style="font-size:28px;font-weight:900">${detailRows.length.toLocaleString('es-HN')}</div><small>RESPUESTAS GUARDADAS</small></div></div>${sinDetalle>0?`<div style="margin-top:10px;padding:9px 10px;border-radius:8px;background:#fff4df;color:#7a4800;font-weight:700">⚠️ ${sinDetalle} encuesta(s) ENVIADA(S) no están apareciendo en el detalle.</div>`:'<div style="margin-top:10px;color:#315e50;font-weight:700">✓ Total de sesiones y detalle de respuestas conciliados.</div>'}`;
