...
export default function UsuariosPage() {
  ...
  const [form, setForm] = useState({
    nombre:'', email:'', rol:'ADMIN_ENCUESTAS', alcance_ecor:'', alcance_municipio:'',
    alcance_establecimiento_codigo:'', alcance_establecimiento_nombre:'', alcance_hospital_nombre:''
  });
  ...
  async function crear(e) {
    ...
    if (!res.ok || !result.ok) setError(result.error || 'No fue posible crear el usuario.');
      ...
      setOk(`Usuario creado como ${form.rol}. Se envió la invitación a ${form.email}.`);
      setForm({ nombre:'', email:'', rol:'ADMIN_ENCUESTAS', alcance_ecor:'', alcance_municipio:'', alcance_establecimiento_codigo:'', alcance_establecimiento_nombre:'', alcance_hospital_nombre:'' });
    }
    setSaving(false);
  }
  ...
  return (
    <main className="shell">
      <section className="loginCard" style={{maxWidth:'780px'}}>
        <div className="badge">ENCUESTAS · OLANCHO</div>
        <h1 style={{fontSize:'34px'}}>Gestionar usuarios</h1>
        ...
        <div style={{background:'#f7faf9',border:'1px solid #dce6e2',borderRadius:'12px',padding:'14px',margin:'18px 0'}}>
          ...
          <p style={{margin:'8px 0 0'}}>El perfil Encuestador se crea exclusivamente desde “Gestionar encuestadores”, porque requiere ID permanente, condición de externo a SESAL y asignación operativa.</p>
        </div>
        ...
        <form onSubmit={crear} style={{display:'grid',gap:'14px'}}>
          ...
          <label><strong>Correo electrónico</strong><input type="email" value={form.email} onChange={e=>setCampo('email',e.target.value)} required style={{width:'100%'}} /></label>
          <label><strong>Perfil</strong>
            <select value={form.rol} onChange={e=>setCampo('rol',e.target.value)} style={{width:'100%',padding:'12px',border:'1px solid #bdcbc6',borderRadius:'9px'}}>
          ...
          {necesitaES && <>
            <label><strong>Código RUPS del establecimiento</strong><input value={form.alcance_establecimiento_codigo} onChange={e=>setCampo('alcance_establecimiento_codigo',e.target.value)} required style={{width:'100%'}} /></label>
            <label><strong>Nombre del establecimiento</strong><input value={form.alcance_establecimiento_nombre} onChange={e=>setCampo('alcance_establecimiento_nombre',e.target.value)} required style={{width:'100%'}} /></label>
          </>}
