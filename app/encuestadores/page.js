...
export default function EncuestadoresPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!perfil || !['ADMIN_GENERAL','ADMIN_ENCUESTAS','JEFE_ENCUESTADORES'].includes(perfil.rol) || !perfil.activo) { router.replace('/panel'); return; }
      if (!active) return;
      setRol(perfil.rol);
  ...
  async function seleccionarMunicipio(valor) {
    setForm(f => ({...f, municipio:valor, establecimiento_codigo:'', establecimiento_nombre:'', hospital:false, tipo_encuesta:'AMBULATORIA'}));
    setError('');
  ...
  function seleccionarEstablecimiento(codigo) {
    ...
    if (!seleccionado) {
      setForm(f => ({...f, establecimiento_codigo:'', establecimiento_nombre:'', hospital:false, tipo_encuesta:'AMBULATORIA'}));
      return;
    }
    const esHospital = !!seleccionado.hospital;
    setForm(f => ({
      ...f,
      establecimiento_codigo: seleccionado.codigo,
      establecimiento_nombre: seleccionado.nombre,
      hospital: esHospital,
      tipo_encuesta: esHospital ? f.tipo_encuesta : 'AMBULATORIA'
    }));
  }
  ...
  async function crearEncuestador(e) {
    ...
    if (!form.municipio || !form.establecimiento_codigo) {
      setError('Seleccione municipio y establecimiento u hospital.'); setCreando(false); return;
    }
    if (form.tipo_encuesta === 'HOSPITALIZACION' && !form.hospital) {
      setError('La encuesta de Hospitalización / Internamiento solo puede asignarse a hospitales.'); setCreando(false); return;
    }
    const supabase = createClient();
    const { data, error: fnError } = await supabase.functions.invoke('crear-encuestador', { body: form });
    if (fnError || !data?.ok) {
      setError(data?.error || fnError?.message || 'No fue posible crear el encuestador.'); setCreando(false); return;
    }
    setMensaje(`Encuestador creado correctamente. ID permanente: ${data.encuestador_id}. Se envió invitación al correo indicado.`);
    setForm(vacio); setEstablecimientos([]); await cargar(); setCreando(false);
  }
  ...
  async function cambiarEstado(item) {
    ...
    const supabase = createClient();
    const { error: rpcError } = await supabase.rpc('cambiar_estado_encuestador', { p_usuario_id:item.usuario_id, p_activo:!item.activo, p_motivo:motivo.trim() || null });
    if (rpcError) { setError(rpcError.message); return; }
    setMensaje(item.activo ? 'Encuestador inactivado.' : 'Encuestador reactivado.'); await cargar();
  }

  async function reasignar(item) {
    const codigo = window.prompt('Nuevo código RUPS / código del establecimiento:', item.establecimiento_codigo || ''); if (codigo === null) return;
    const nombre = window.prompt('Nuevo establecimiento u hospital:', item.establecimiento_nombre || ''); if (nombre === null) return;
    const tipo = window.prompt('Tipo de encuesta: AMBULATORIA o HOSPITALIZACION', item.tipo_encuesta || 'AMBULATORIA'); if (tipo === null) return;
    const hospital = window.confirm('¿La nueva asignación corresponde a un hospital?');
    if (tipo.trim().toUpperCase() === 'HOSPITALIZACION' && !hospital) { setError('Hospitalización / Internamiento solo puede asignarse a hospitales.'); return; }
    const motivo = window.prompt('Motivo de la reasignación:'); if (motivo === null) return;
    const supabase = createClient();
    const { error: rpcError } = await supabase.rpc('reasignar_encuestador', {
      p_usuario_id:item.usuario_id, p_tipo_encuesta:tipo.trim().toUpperCase(), p_establecimiento_codigo:codigo.trim(), p_establecimiento_nombre:nombre.trim(), p_hospital:hospital, p_motivo:motivo.trim()
    });
    if (rpcError) { setError(rpcError.message); return; }
    setMensaje('Encuestador reasignado. La asignación anterior quedó conservada en el historial.'); await cargar();
  }
  ...
  return (
    <main className="shell"><section className="loginCard" style={{maxWidth:'900px'}}>
      ...
      <div style={{background:'#f7faf9',border:'1px solid #dce6e2',borderRadius:'12px',padding:'14px',margin:'18px 0'}}><strong>Reglas vigentes</strong><p style={{margin:'8px 0 0'}}>Cada encuestador debe ser externo a SESAL, conservar un ID único permanente y tener establecimiento u hospital asignado. Hospitalización / Internamiento solo puede asignarse a hospitales.</p></div>

      {puedeCrear && <form onSubmit={crearEncuestador} style={{border:'2px solid #17634e',borderRadius:'14px',padding:'18px',margin:'20px 0'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:'12px'}}>
          ...
          <label>Tipo de encuesta<select value={form.tipo_encuesta} onChange={e=>cambia('tipo_encuesta',e.target.value)} style={{width:'100%',padding:'11px',marginTop:'5px'}}><option value="AMBULATORIA">Atención Ambulatoria</option>{form.hospital && <option value="HOSPITALIZACION">Hospitalización / Internamiento</option>}</select></label>
        </div>
        ...
        <p style={{fontSize:'13px',color:'#647a74'}}>El municipio determina automáticamente las unidades disponibles. Hospitalización solo se habilita al seleccionar un hospital.</p>
      </form>}
      ...
      {!loading && items.map((item) => <div key={`${item.usuario_id}-${item.encuestador_id}-${item.fecha_asignacion}`} style={{border:'1px solid #dce6e2',borderRadius:'12px',padding:'16px',margin:'14px 0'}}>
