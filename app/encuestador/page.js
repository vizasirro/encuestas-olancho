...
export default function EncuestadorPage() {
  ...
  function continuarEncuesta(){
    if(!encuestaEnCurso?.url)return;
    router.push(encuestaEnCurso.url);
  }

  function solicitarConsentimiento() {
    if(encuestaEnCurso){setError('Ya existe una encuesta en curso. Debe continuarla y finalizarla antes de iniciar otra.');return;}
    if (!seleccion) { setError('Seleccione la designación con la que realizará la encuesta.'); return; }
    setError(''); setMensaje(''); setMostrarConsentimiento(true);
  }

  function noAceptaParticipar() {
    setMostrarConsentimiento(false); setError('');
  ...
  async function aceptaParticipar() {
    if(encuestaEnCurso){setMostrarConsentimiento(false);setError('Ya existe una encuesta en curso. Continúela antes de iniciar otra.');return;}
    ...
    const url=`/encuestas/${sesion.encuesta_codigo}?sesion=${sesion.sesion_id}&folio=${encodeURIComponent(sesion.folio)}&modo=aplicacion`;
    try{localStorage.setItem(CURSO_KEY,JSON.stringify({url,sesion_id:sesion.sesion_id,folio:sesion.folio,codigo:sesion.encuesta_codigo}));}catch{}
    router.push(url);
  }
  ...
  return <main className="shell"><section className="loginCard" style={{maxWidth:'760px'}}>
    <div className="badge">ENCUESTAS · OLANCHO</div><h1>Encuestador</h1>
